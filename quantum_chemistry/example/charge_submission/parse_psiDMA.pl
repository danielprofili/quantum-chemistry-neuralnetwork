...
if( $flag == 0 ) { die "error parsing psi4 dma output\n" }

<ifile>;$_=<ifile>;my@atomtype;my@xyz;
 until( /^\s*$/ )
{my@data=split;
 push(@atomtype, $data[0]);
 push(@xyz,[ $data[1] , $data[2], $data[3] ]);
$_=<ifile>;
 }
$atoms=@atomtype;
...
while(<ifile>)
{
 if(/Multipole moments/) { $flag=1 ; last }
last if eof();
}
if( $flag == 0 ) { die "error parsing psi4 dma output\n" }
